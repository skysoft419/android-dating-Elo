...
public class BackgroundAwareLayout extends RelativeLayout {
    private int childId=0;
    private View childView;
    private Paint eraser;
    private float radius=0;
    private RectF childRect=new RectF();
    public BackgroundAwareLayout(Context context,  AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        setUp(attrs,context);

    }

    public BackgroundAwareLayout(Context context) {
        super(context);
    }

    public BackgroundAwareLayout(Context context, AttributeSet attrs) {
        super(context, attrs);
        setUp(attrs,context);
    }
    ...
    private void setUp(AttributeSet attrs, Context context){
        ...
        this.radius= (float) context.getResources().getDimensionPixelSize(R.dimen.onboarding_bubble_radius);
        if (childId != 0){
            ta.recycle();
            setupEraser(context);
            return;

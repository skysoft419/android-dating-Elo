...
public class StoriesProgressView extends LinearLayout {
    ...
    public void skipStory(int index){

        PausableProgressBar p = progressBars.get(index);
        p.setMax();
        if (index != 0) {
            PausableProgressBar previous = progressBars.get(index - 1);
            previous.setMin();
        }
    }
    public void reverseStory(int index){

        PausableProgressBar p = progressBars.get(index+1);
        p.setMin();
        PausableProgressBar previous=progressBars.get(index);
        previous.setMax();
    }
    /**
